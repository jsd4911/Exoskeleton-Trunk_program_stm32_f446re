...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : Main program body for STM32N657X0 BNO085 Scan Test (I2C4 Edition)
  ******************************************************************************
  * @attention
  *
...
static void MX_I2C4_Init(void); // 👈 宣告升級為 I2C4
static void MX_USART1_UART_Init(void);
static void SystemIsolation_Config(void);
/* USER CODE BEGIN PFP */
void I2C4_Scan(void); // 👈 提供帶有 5ms 延遲保護的 I2C4 總線掃描宣告
/* USER CODE END PFP */
...
// 👈 補回帶有「暫存器緩衝恢復防護」的 I2C4 總線地址掃描器實體定義
void I2C4_Scan(void) {
    printf("[I2C4] 開始掃描總線裝置...\r\n");
    uint8_t devices_found = 0;

    for (uint16_t i = 1; i < 128; i++) {
        // 使用 IsDeviceReady 對 I2C4 進行探測，確認裝置是否有發出 ACK 響應
        ...
        // STM32N6 運作頻率高達 800MHz。加上這 5ms 能保證高速狀態暫存器乾淨復位，防止 Ghost ACK。
        HAL_Delay(5);
    }
...
int main(void)
{
  ...
  // 🚀 【終極防護：手動打通 RIF 屏障】
  // 強制將 I2C4 所屬的 PE13 與 PE14 引腳降級為 NSEC (Non-Secure, 非安全屬性)
  // 這樣無論主程式在哪個安全世界運行，都能 100% 成功對這兩根引腳進行 I2C 通訊！
  ...
  // 1. 【防當機神器】強制重置 BNO085 (使用 N657X0-Q 的 D8/PD12 腳位)
  HAL_GPIO_WritePin(GPIOD, GPIO_PIN_12, GPIO_PIN_RESET); // RST 拉低
  HAL_Delay(50);
  HAL_GPIO_WritePin(GPIOD, GPIO_PIN_12, GPIO_PIN_SET);   // RST 拉高 (放開)
  HAL_Delay(500);                                       // 等待感測器完全開機

  // 2. 掃描 BNO085 的兩個可能位址 (使用 N657X0-Q 的 I2C4)
  printf("正在掃描 I2C 總線...\r\n");

  if (HAL_I2C_IsDeviceReady(&hi2c4, (0x4A << 1), 5, 100) == HAL_OK) {
      printf("太棒了！SUCCESS: BNO085 found at 0x4A!\r\n");
  }
  else if (HAL_I2C_IsDeviceReady(&hi2c4, (0x4B << 1), 5, 100) == HAL_OK) {
      printf("太棒了！SUCCESS: BNO085 found at 0x4B!\r\n");
  }
  else {
      printf("錯誤：找不到感測器 (ERROR: No device detected)。請檢查接線與上拉電阻。\r\n");
  }
  /* USER CODE END 2 */
